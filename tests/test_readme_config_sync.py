...
def _parse_parameter_table() -> dict[str, tuple[str, str]]:
    """Return default and description cells indexed by parameter name.

    Return {key: (default_cell, description_cell)} for every row under "Complete
    Parameter List".
    """
...
def test_every_default_key_is_documented():
    ...
    missing = set(src.config._DEFAULTS) - documented.keys()
    assert not missing, (
...
def test_no_stale_or_typo_rows():
    """Reject README parameters absent from the configuration defaults.

    A row for a key that no longer exists in _DEFAULTS is either a typo or
    leftover from a removed setting — either way, it's actively wrong.
    """
    documented = _parse_parameter_table()
    stale = documented.keys() - set(src.config._DEFAULTS)
    assert not stale, (
        "README documents parameters that don't exist in src.config._DEFAULTS: "
        f"{sorted(stale)} — fix the key name or remove the row"
    )


def test_documented_defaults_match_source():
    """Keep README default values aligned with the runtime defaults.

    The value shown in the table must be the real default, not a stale or
    aspirational one.
    """
    documented = _parse_parameter_table()
    mismatches = []
    for key, default_val in src.config._DEFAULTS.items():
        ...
        if expected not in cell:
            mismatches.append(
                f"{key}: README says {cell!r}, _DEFAULTS has {default_val!r}"
            )
...
def test_project_forbidden_keys_are_flagged_in_readme():
    """Document every restriction on project-level configuration.

    The three keys that a project-level .token-saver.json cannot set must say so
    in their README row — this is a security property, not trivia.
    """
    documented = _parse_parameter_table()
    for key in src.config._PROJECT_FORBIDDEN_KEYS:
        assert key in documented, (
            f"{key} is missing from the README table entirely"
        )
        _default_cell, description = documented[key]
        assert (
            "not project config" in description
            or "global config" in description
        ), (
            f"{key}'s README row doesn't mention the project-config "
            f"restriction: {description!r}"
        )
